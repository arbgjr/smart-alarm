...
namespace SmartAlarm.AlarmService.Controllers
{
    ...
    public class AlarmsController : ControllerBase
    {
        ...
        /// <summary>
        /// Processa alarme usando sistema distribuído
        /// </summary>
        /// <param name="alarmId">ID do alarme</param>
        /// <param name="request">Configurações de processamento</param>
        /// <returns>Resultado do processamento distribuído</returns>
        [HttpPost("{alarmId:guid}/process-distributed")]
        public async Task<IActionResult> ProcessDistributedAlarm(Guid alarmId, [FromBody] ProcessDistributedAlarmRequest request)
        {
            using var activity = _activitySource.StartActivity("AlarmsController.ProcessDistributedAlarm");
            var stopwatch = Stopwatch.StartNew();
            ...
            try
            {
                ...
                activity?.SetTag("operation", "process_distributed_alarm");

                _logger.LogInformation("Processamento distribuído solicitado para alarme {AlarmId} - UseQueue: {UseQueue} - CorrelationId: {CorrelationId}",
                    alarmId, request.UseQueue, _correlationContext.CorrelationId);

                var command = new ProcessDistributedAlarmCommand(
                    alarmId,
                    request.UserId,
                ...
                _logger.LogInformation("Processamento distribuído do alarme {AlarmId} concluído - Sucesso: {Success} em {Duration}ms",
                    alarmId, result.Success, stopwatch.ElapsedMilliseconds);
            ...
            catch (Exception ex)
            {
                ...
                _meter.IncrementErrorCount("controller", "process_distributed_alarm", "exception");
        ...
        /// </summary>
        /// <param name="queueName">Nome da fila (opcional)</param>
        /// <returns>Estatísticas da fila</returns>
        [HttpGet("queue/statistics")]
        public async Task<IActionResult> GetQueueStatistics([FromQuery] string queueName = "default")
        {
            using var activity = _activitySource.StartActivity("AlarmsController.GetQueueStatistics");
            var stopwatch = Stopwatch.StartNew();
            ...
            try
            {
                ...
                _logger.LogInformation("Obtendo estatísticas da fila {QueueName} - CorrelationId: {CorrelationId}",
                    queueName, _correlationContext.CorrelationId);

                // Usar uma query específica para estatísticas
                var query = new GetQueueStatisticsQuery(queueName);
                var result = await _mediator.Send(query);

                stopwatch.Stop();
                _meter.RecordRequestDuration(stopwatch.ElapsedMilliseconds, "get_queue_statistics", "success", "200");

                _logger.LogInformation("Estatísticas da fila {QueueName} obtidas em {Duration}ms",
            ...
            catch (Exception ex)
            {
                ...
                _meter.IncrementErrorCount("controller", "queue_statistics", "exception");
        ...
        /// <summary>
        /// Lista alarmes ativos em processamento
        /// </summary>
        /// <returns>Lista de alarmes em processamento</returns>
        [HttpGet("processing/active")]
        public async Task<IActionResult> GetActiveProcessingAlarms()
        {
            using var activity = _activitySource.StartActivity("AlarmsController.GetActiveProcessingAlarms");
            var stopwatch = Stopwatch.StartNew();
            ...
            try
            {
                ...
                _logger.LogInformation("Listando alarmes em processamento ativo - CorrelationId: {CorrelationId}",
                    _correlationContext.CorrelationId);

                var query = new GetActiveProcessingAlarmsQuery();
                var result = await _mediator.Send(query);

                stopwatch.Stop();
                _meter.RecordRequestDuration(stopwatch.ElapsedMilliseconds, "get_active_processing_alarms", "success", "200");

                _logger.LogInformation("Lista de alarmes em processamento obtida - Count: {Count} em {Duration}ms",
                    result.ActiveAlarms.Count(), stopwatch.ElapsedMilliseconds);
            ...
            {
                stopwatch.Stop();
                _meter.RecordRequestDuration(stopwatch.ElapsedMilliseconds, "get_active_processing_alarms", "error", "500");
                _meter.IncrementErrorCount("controller", "active_processing_alarms", "exception");

                activity?.SetTag("error", true);
                activity?.SetTag("error.message", ex.Message);

                _logger.LogError(ex, "Erro ao listar alarmes em processamento - CorrelationId: {CorrelationId}",
                    _correlationContext.CorrelationId);
    ...
    /// <summary>
    /// Modelo para processamento distribuído de alarme
    /// </summary>
    public class ProcessDistributedAlarmRequest
    {
        public Guid UserId { get; set; }
